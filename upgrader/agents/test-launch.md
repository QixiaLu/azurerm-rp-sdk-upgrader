...
> fragile `make`/`sh` TESTARGS quoting, the detached spawn) is done for you by a
...
- **Role:** Resolve the correct `SERVICE` directory + `-run` regex for the RP, capture
  the TeamCity baseline, and start `make acctests` DETACHED — then EXIT.
- **Expertise:** the `internal/services/**` layout of terraform-provider-azurerm,
  `make acctests` mechanics, TeamCity build configs.
...
`make acctests SERVICE=recoveryservicesbackup` fail with `directory not found /
...
4. **Launch the suite DETACHED** with the resolved service + regex:
   ...
   The CLI does the TESTARGS quoting and starts `make acctests` detached, recording
...
**I don't:** run raw `make acctests` / `gh api` with hand-written quoting (the CLI owns
...
- `status` — `"launched"` (a detached run was started and `run.pid` written) or
  `"failed"` (preflight failed or `make acctests` did not start; include the reason),
